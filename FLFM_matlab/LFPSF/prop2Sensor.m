...
if (idealSampling)
    if (z == 0)
        ...
        ideal_samples_no = ideal_samples_no + (1-mod(ideal_samples_no,2)); %odd
        rate = [Lx, Ly]./ideal_samples_no; %lambda*sqrt(z^2 +(Lx/2)^2)/Lx
        
        % spacial frequencies in x and y direction
        du = 1./(ideal_samples_no(1)*single(rate(1))); %1/Lx
