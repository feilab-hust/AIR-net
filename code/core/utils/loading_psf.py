...
if __name__=='__main__':
    ...
    # 2501 7views
    # centercrops =[[817,1000],[817,1500],[1250,751],[1250,1250],[1250,1749],[1683,1000],[1683,1500]]
    ...
    #     [0, 0, 0],
    #                        ],np.float32)
    costumed_psf=imageio.imread(r'J:\Nerf_test\Data\FLFM_View7_livingcell\FLFM_live\psf_step0.2\deblur_center_mito\psf\centerpsf.tif')
    ...
    # #
    #
    np.save(os.path.join(out_dir,'new_psf_pattern.npy'),new_psf.cpu().numpy())
    ...
    # view_stack = imageio.imread(r'E:\NetCode\Nerf_test\LF_INR_MixedFusion\data\LF\view7_matrix_60x\train\view\EqualIntensity_ref_1/viewstack.tif')
    # view_stack = np.asarray(view_stack,np.float32)
    # view_stack = view_stack[...,np.newaxis]
    # view_stack = torch.from_numpy(view_stack).to(DEVICE)
