...
def pad_caculate(padding, n, kernel_size,signal_size, stride):
    if padding == 'valid':
        padingH=0
        padingW=0
    ...
    if padding=='same':
        ...
        h2=signal_size[2]
        w2=signal_size[3]
        padingH=int(((h2-1)/stride-h2+h1)/2)
        padingW = int(((w2 - 1) / stride - w2 + w1) / 2)
    return  tuple((padingH,padingW))
...
def fft_conv(
    ...
    s_size=signal.shape
    k_size=kernel.shape
    padding_=pad_caculate(padding, n , k_size , s_size , stride)
    ...
    # pad the kernel internally according to the dilation parameters    kernel = torch.kron(kernel, offset)[(slice(None), slice(None)) + cutoff]
...
def cfft_conv(
    ...
    # Perform fourier convolution -- FFT, matrix multiply, then IFFT
    # signal_ = signal_.reshape(signal_.size(0), groups, -1, *signal_.shape[2:])
    signal_fr = fftn(signal_, dim=tuple(range(2, signal.ndim)))
