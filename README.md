...
ideal PSF "HS.mat" by running the ./FLFM_matlab/PSF_simulation.m file in MATLAB, based on your system parameters.
...
Users can modify the parameters following the code annotations in "main.py". Typing the following command in console to start training.
  ```
  python ./code/main.py --flagfile=code/configs/exp_train.cfg
  ```
 ...
 The aberrated light field image, along with the ideal PSF(HS.mat), were then used as inputs to train the AIR-net.
  ```
  python ./code/main.py --flagfile=code/configs/sim_train.cfg
  ```
...
* Commands:
  ```
  python ./code/main.py --flagfile=code/configs/exp_test.cfg
  ```
#### Example 1: Simulated data testing
* Commands:
  ```
  python ./code/main.py --flagfile=code/configs/sim_test.cfg
  ```
